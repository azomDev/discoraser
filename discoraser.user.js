// ==UserScript==
// @name         Discoraser
// @namespace    https://azom.dev/discoraser
// @version      1.0.0
        ...
        return new Promise((resolve, reject) => {
            ...
            const check = () => {
                ...
                if (Date.now() - start > timeout)
                    return reject(
                        new Error(`Timeout: ${fn.toString().slice(0, 80)}`),
                    );
        ...
        try {
            ...
            while (running) {
                ...
                if (toDelete.length > 0) {
                    ...
                    log(
                        `Removing ${myReactions.length} reactions on screen...`,
                    );
                    noNewContentCount = 0;
                    ...
                    const scrollMoved =
                        Math.abs(scroller.scrollTop - prevScrollTop) > 5;
                    ...
                    if (!scrollMoved && !heightGrew) {
                        ...
                        if (noNewContentCount >= 3) {
                            log(
                                "Reached the top of the channel. No more messages to load.",
                            );
                            break;
                        }
                        scroller.scrollTop -= CONFIG.scrollAmount;
